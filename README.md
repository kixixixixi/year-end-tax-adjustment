...
│   └── SocialInsuranceEstimator.tsx # 社会保険料推計ツール
├── hooks/                        # カスタムフック
│   └── useTaxCalculation.ts      # 税計算ロジック
├── constants.ts                  # 定数定義
├── types.ts                     # 型定義
├── tax-calculator.ts            # 税計算エンジン
├── social-insurance-calculator.ts # 社会保険料計算エンジン
├── layout.tsx                   # ルートレイアウト
└── page.tsx                    # メインページ
```
...
社会保険料推計ツールにより、以下の保険料を詳細計算：
- **健康保険料** - 都道府県別料率に対応（協会けんぽ2024年度）
- **介護保険料** - 40歳以上対象（料率1.23%）
- **厚生年金保険料** - 料率18.3%
- **雇用保険料** - 一般事業0.6%
